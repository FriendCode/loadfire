...
GroupBalancer.prototype.handleError = function(err, req, res, backend) {
    var target = [backend.host, backend.port].join(':') + req.url;
...
GroupBalancer.prototype._handleHttp = function(req, res, backend, buffer) {
    ...
    return retry(self._proxyToBackend, this.config.retries, this.config.retryTimeout)(req, res, backend, buffer)
    .fail(function(err) {
